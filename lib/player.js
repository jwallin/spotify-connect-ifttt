...
function setVolumeTo(token, volume) {
  const query_params = `?volume_percent=${volume}`
  const url = VOLUME_URL + query_params;
...
exports.setVolume = function setVolume(volume) {
  console.log('Setting volume to', volume);

  return getToken()
    .then(token => setVolumeTo(token, volume));
};
